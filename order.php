<?php
	...
	$query = $self->selectAllByUserID($user);
	// print_r($query); die();
    $result = mysqli_query($connection, $query);
?>
...
<div class="col-sm-12">
	<table class="table table-hover">
		<div class="etri-form">
			<br><br><br><h1><b>PEMESANAN ANDA</h1>
		</div>
		...
		<?php while ($row = mysqli_fetch_array($result))
			{ ?>
				<tr class="danger" height="20px">
					...
					<!-- <td>
						<a href="order.php?order=<?php echo $row["id_booking"]?>" onclick="JavaScript: return confirm('Konfirmasi pemesanan Truck dengan Nomor Polisi <?= $row['no_polisi']; ?>?')">   Pesan</a>
					</td> -->
				</tr>
				<?php $no++; ?>
			<?php  } ?>
